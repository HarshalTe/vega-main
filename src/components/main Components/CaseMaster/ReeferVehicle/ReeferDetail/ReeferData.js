...
function ReeferData(props) {
  ...
  const handleSubmitNew = (values, { setSubmitting }) => {
    ...
    user.append("case_id", values.case_id);
    user.append("sheet", values.sheet);

 

    console.log("Data of cases:", user, values.sheet,values.case_id);

    authAxios
      .post("sensors-upload", user)
      .then((res) => {
        console.log("res of Sensors upload", res);
        swal("Successfully Sensors uploaded Excel").then(() => {
          // props.onRawDataGetData(data);
          props.sensorsGetData(data);
          // props.testsEditGetData(data);
          setSubmitting(false);
      ...
      <CardHeader className="bg-warning text-white">
        <div className="d-flex" style={{"justifyContent":"space-between"}}>
          ...
          <a href={`${imageUrl}demo/data_loggers.xlsx`} download="DemoData">
            <Button className="btn mr-3">Demo Data For loggers</Button>
          </a>
          <a href={`${imageUrl}demo/sensor-import-demo.xlsx`} download="DemoData">
            <Button className="btn mr-3">Demo Data For Naming Convention</Button>
          </a>
            </div>
        </div>
                ...
                {(formProps) => {
                  return (
                    ...
                    <Form>
                      <Row className="form-group">
                            <Col md={6}>
                            <Label for="file">Upload Excel For Loggers</Label>
                ...
                {(formProps) => {
                  return (
                    ...
                    <Form>
                      <Row className="form-group">
                            <Col md={6}>
                            <Label for="file">Upload Excel For Naming Convention</Label>
                            <div style={sensors?.length >= 1 ?{color:"red"}:{display:"none"}}>Alert: Already uploded Excel</div>
                          <InputGroup>
                            ...
                            <input
                              ...
                              onChange={(event) => {
                                ...
                                className={
                                "form-control" +
                                (formProps.errors.sheet && formProps.touched.sheet
                                  ? " is-invalid"
                                  : "")
                              }
                              />
...
export default connect(mapStateToProps, mapDispatchToProps)(ReeferData);
