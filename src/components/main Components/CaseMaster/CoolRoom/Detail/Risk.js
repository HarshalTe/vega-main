...
let data = {
  ...
  // // console.log("object12345",Atime,Atime.length)
  ...
  // console.log(RavSetTime, Math.round(RavSetTime) ,"bbb")
  ...
  props.cycle?.cycle?.length > 0
    ...
    : [];
                  console.log("12345",doorOpenGraphData1?.dcp_on_time,doorOpenGraphData1,doorOpenGraphData2,doorOpenGraphData3,props.cycle?.cycle?.length)
...
props.cycle?.cycle?.length > 0
  ...
  props.cycle?.cycle?.length > 0
      ? props.cycle?.cycle?.filter(
        ...
        : [];
        console.log("12345",powerOnGraphData1,powerOnGraphData1,powerOnGraphData2,powerOnGraphData3,props.cycle?.cycle?.length)
...
// console.log("object12345",power1,power2,[addTimes(powerOnGraphData1?.set_limit_cross_out_time, powerOnGraphData1?.dcp_on_time),addTimes(powerOnGraphData1?.set_limit_cross_in_time,powerOnGraphData1?.do_off_time)],powerOnGraphData1?.id>0)
          ...
          .map((user) => {
            return (
              <Formik
                key={user.id}
                initialValues={{
                  // obeservation_remark:`1. During Door Open Test Cycle, when Door was Opened, Excursion was observed after ${avTime} mins and when Power Supply was Switched ON, Normal Operation Range was recovered within ${RavSetTime} mins., \n\n 2.During Power Failure Test Cycle, when Power Supply was Switched OFF, Excursion was observed after ${pTime} mins and when Power Supply was Switched ON, Normal Operation Range was recovered within ${RavSetTime} mins.`,
                  obeservation_remark: 
                  user.obeservation_remark
                    ? user.obeservation_remark
                    : 
                    user.type_of_room == "Cold Room" || user.type_of_room=="Deep Freezer Room" || user.type_of_room=="Cool Room"|| user.type_of_room=="Ambient Room"|| user.type_of_room=="Walk-In Deep Freezer"|| user.type_of_room=="Walk-In Cold Room"|| user.type_of_room=="Warehouse"
                    ? `1. During Door Open Test Cycle, when Door was Opened, Excursion was observed after ${ Math.round(AdoorF)} mins and when Power Supply was Switched ON, Normal Operation Range was recovered within ${ Math.round(AdoorL)}  mins. \n\n 2.During Power Failure Test Cycle, when Power Supply was Switched OFF, Excursion was observed after ${ Math.round(ApowerF)} mins and when Power Supply was Switched ON, Normal Operation Range was recovered within ${ Math.round(ApowerL)} mins.`
                    : user.type_of_room == "ViSi Cooler" || user.type_of_room == "Autoclave" || user.type_of_room == "ETO" || user.type_of_room == "Furnace" || user.type_of_room == "Oven" || user.type_of_room == "Thermal Box" || user.type_of_room == "Refrigerator" || user.type_of_room == "Deep Freezer" || user.type_of_room == "Stability Chamber" 
                    ? `1. During Door Open Test Cycle, when Door was Opened, Excursion was observed after ${ Math.round(AdoorF)} mins and when Power Supply was Switched ON, Normal Operation Range was recovered within ${ Math.round(AdoorL)} mins. \n\n 2.During Power Failure Test Cycle, when Power Supply was Switched OFF, Excursion was observed after ${ Math.round(ApowerF)} mins and when Power Supply was Switched ON, Normal Operation Range was recovered within ${ Math.round(ApowerL)} mins.`
                    : "",
