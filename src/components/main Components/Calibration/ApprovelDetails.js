...
function ApprovelDetails(props) {
...
let formattedDateTime = currentDate.toLocaleString('en-GB', options);
  const handleSubmit = ({ setSubmitting }) => {
    // assignCase(values);
    let user ={
        equipment:props?.login?.login?.user?.name,
        certificate_valid_upto:formattedDateTime
    }
    // props.detailCasesData(data, toggle);
    console.log("object",formattedDateTime,user)
    props.onUpdateCalibration(data, user, toggle);
    setSubmitting(true);
  };
  const handleSubmit2 = (values, { setSubmitting }) => {
    // assignCase(values);
    let user ={
        env_condition:props?.login?.login?.user?.name,
        calibration_frequency:formattedDateTime
    }
    // props.detailCasesData(data, toggle);
    console.log("object",formattedDateTime,user)
    props.onUpdateCalibration(data, user, toggle);
...
const mapDispatchToProps = (dispatch) => {
  return {
    onUpdateCalibration: (data, user, toggle) => dispatch(actions.updateCalibration(data, user, toggle)),
...
export default connect(mapStateToProps, mapDispatchToProps)(ApprovelDetails);
